...
async function start() {
    ...
    bot.ev.on("messages.upsert", async ({ messages }) => {
        ...
        if (msg.key.fromMe && message?.toLocaleLowerCase() === 'atendimento finalizado') {
            const userCache = cacheUserData.find(u => u.lid === fromLid);
            DATA_CLIENTS.updateUserData(fromLid as string, { step: 'INITIAL', username: userCache?.username || '' });
        }
        ...
        if (user.step !== 'COLLECT_NAME' && user.step !== 'HUMAN_SERVICE') {
            ...
            switch (message.toLocaleLowerCase()) {
                case 'menu': case 'ajuda':
                    ...
                    isCommand = true;
                    break;
                case 'quero um atendente': case 'atendente': case 'humano': case 'preciso de ajuda': case 'falar com atendente':
                    await askAttendant();
                    isCommand = true;
                    break;
                default:
        ...
        switch (user.step.toUpperCase()) {
            case 'INITIAL':
                ...
                DATA_CLIENTS.updateUserData(fromLid as string, { step: 'MENU', username: user.username });
                break
            case 'HUMAN_SERVICE':
            ...
            case 'COLLECT_NAME':
                ...
                DATA_CLIENTS.updateUserData(fromLid as string, { step: 'MENU', username });
